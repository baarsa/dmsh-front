...
export class LoadsInfoVM {
    ...
    private _calculateItems() {
        const loads = this._schedule.loads.filter(load => load.teacher === this._teacherId);
        const lessons = this._schedule.lessons.filter(lesson => lesson.teacher === this._teacherId);
        ...
        this._items = loads.map(load => {
            const lessonsForLoad = lessons.filter(lesson =>
            ...
            const halfHoursForSubject = load.subject === pupil.specialSubject
            ...
            return {
                ...
                assignedCount: lessonsForLoad.reduce((acc, lesson) => acc + lesson.end - lesson.start , 0),
