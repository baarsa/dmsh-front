...
export const LoadsInfo = observer(({ vm }: Props) => {
    return <div className={cn({ open: vm.isOpen })}>
        <div className={cn('top')} onClick={() => {vm.isOpen = !vm.isOpen}}>
            { vm.title }
        ...
        <Collapse in={vm.isOpen}>
            <div className={cn('content')}>
                {
                    vm.items.map((item, i) => <div key={i}>{ item.text }: {item.assignedCount} / {item.totalCount}</div>)
                }
            </div>
        </Collapse>
    </div>
